...
public:
  Runtime() {
    ...
    NSString *source = [[NSString alloc]
        initWithBytes:detail::kKernelSource
...
id<MTLBuffer> MakeBuffer(id<MTLDevice> device, std::size_t bytes) {
  const NSUInteger length = static_cast<NSUInteger>(std::max(bytes, sizeof(float)));
  id<MTLBuffer> buffer =
...
void Workspace::Reserve(const btrc::Plan &plan, std::size_t states,
  ...
  @autoreleasepool {
    if (states == 0 || batch == 0)
      throw std::invalid_argument("Metal state and batch counts must be nonzero");
    ...
    storage.rakes = MakeDataBuffer(runtime.device(),
                                   plan.rakes());
    storage.combinations = MakeDataBuffer(
        runtime.device(), plan.branch_combinations());
    storage.absorptions = MakeDataBuffer(
        runtime.device(), plan.branch_absorptions());
    storage.compressions = MakeDataBuffer(
        runtime.device(), plan.compressions());
    ...
    const std::size_t path_batches =
        plan.num_compressions() == 0 ? 1 : batch;
    storage.paths = MakeBuffer(
        runtime.device(),
        CheckedProduct(
            {path_batches, plan.num_edges(), matrix_size, sizeof(float)},
    ...
    storage.node_scales = MakeBuffer(
        runtime.device(),
    ...
    storage.path_scales = MakeBuffer(
        ...
        CheckedProduct({batch, plan.num_edges(), sizeof(float)},
                       "Metal path scales"));
    storage.branch_scales = MakeBuffer(
        runtime.device(),
        CheckedProduct({batch, plan.num_branches(), sizeof(float)},
                       "Metal branch scales"));
    storage.output = MakeBuffer(
        runtime.device(), CheckedProduct({batch, sizeof(float)}, "Metal output"));
  }
}
...
tree_hmm::PartitionView Run(tree_hmm::BatchedModelView model,
  ...
  @autoreleasepool {
    ...
    if (model.node_potentials.size() != expected_nodes ||
      ...
      throw std::invalid_argument("Metal input shapes do not match the workspace");
    }
    ...
    for (const btrc::PrimitiveBatch &primitive_batch :
      ...
      case btrc::Primitive::kRake:
        [encoder setComputePipelineState:runtime.rake()];
        ...
        [encoder dispatchThreads:MTLSizeMake(model.states, primitive_batch.count,
                                             model.batch)
            threadsPerThreadgroup:MTLSizeMake(model.states, 1, 1)];
        break;
      case btrc::Primitive::kBranchCombination:
        [encoder setComputePipelineState:runtime.combine()];
      ...
      case btrc::Primitive::kBranchAbsorption:
        [encoder setComputePipelineState:runtime.absorb()];
      ...
      case btrc::Primitive::kCompression: {
        [encoder setComputePipelineState:runtime.compress()];
        ...
        [encoder setThreadgroupMemoryLength:
                     (2 * matrix_size + model.states + 1) * sizeof(float)
                                  atIndex:0];
