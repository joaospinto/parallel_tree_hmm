...
constexpr double kTolerance =
    std::is_same_v<tree_hmm::Scalar, float> ? 2e-5 : 1e-11;
constexpr double kLongTolerance =
    std::is_same_v<tree_hmm::Scalar, float> ? 2e-4 : 1e-10;
...
tree_hmm::Marginals
BruteForce(const btrc::Plan &plan, std::size_t states,
           const std::vector<tree_hmm::Scalar> &node_potentials,
           const std::vector<tree_hmm::Scalar> &edge_potentials) {
  ...
  for (tree_hmm::Scalar &value : result.nodes)
    value /= result.partition;
  for (tree_hmm::Scalar &value : result.edges)
    value /= result.partition;
...
tree_hmm::Scalar
AssignmentWeight(const btrc::Plan &plan, std::size_t states,
                 const std::vector<tree_hmm::Scalar> &node_potentials,
                 const std::vector<tree_hmm::Scalar> &edge_potentials,
                 std::span<const std::size_t> assignment) {
  Check(assignment.size() == plan.num_nodes());
  tree_hmm::Scalar result = 1.0;
  for (std::size_t node = 0; node < plan.num_nodes(); ++node) {
...
int main() {
  ...
  const std::vector<tree_hmm::Scalar> edges{
  ...
  const std::vector<tree_hmm::Scalar> zero_nodes{
      0.55, 0.45, 1.0, 0.0, 0.0, 1.0, 0.7, 0.3, 0.0, 1.0,
  };
  const std::vector<tree_hmm::Scalar> zero_edges{
  ...
  const std::vector<tree_hmm::Scalar> midpoint_uniforms(plan.num_nodes(), 0.5);
  ...
  const std::vector<tree_hmm::Scalar> tie_nodes(plan.num_nodes() * kStates,
                                                1.0);
  const std::vector<tree_hmm::Scalar> tie_edges(
      plan.num_edges() * kStates * kStates, 1.0);
  ...
  std::vector<tree_hmm::Scalar> sampled_nodes(expected.nodes.size(), 0.0);
  std::vector<tree_hmm::Scalar> sampled_edges(expected.edges.size(), 0.0);
  ...
  for (std::size_t draw = 0; draw < kSamples; ++draw) {
    for (tree_hmm::Scalar &uniform : uniforms)
  ...
  const std::vector<tree_hmm::Scalar> long_nodes(kLongNodes, 0.5);
  const std::vector<tree_hmm::Scalar> long_edges(kLongNodes - 1, 1.0);
  const double log_partition =
      tree_hmm::LogPartitionFunction({long_plan, 1, long_nodes, long_edges});
  Check(Near(log_partition, kLongNodes * std::log(0.5), kLongTolerance));
  ...
  Check(Near(long_marginals.log_partition, log_partition, kLongTolerance));
  ...
  Check(Near(long_maximum.log_weight, log_partition, kLongTolerance));

  const std::vector<tree_hmm::Scalar> long_binary_nodes(kLongNodes * 2, 0.5);
  std::vector<tree_hmm::Scalar> long_binary_edges((kLongNodes - 1) * 4);
  ...
  Check(Near(long_binary_marginals.log_partition,
             (kLongNodes - 1) * std::log(0.5), kLongTolerance));
  ...
  for (std::size_t edge = 0; edge < kLongNodes - 1; ++edge) {
    const tree_hmm::Scalar *joint =
        long_binary_marginals.edges.data() + edge * 4;
    Check(Near(std::accumulate(joint, joint + 4, 0.0), 1.0, kLongTolerance));
    ...
    Check(Near(joint[0] + joint[1], long_binary_marginals.nodes[parent * 2],
               kLongTolerance));
    Check(Near(joint[2] + joint[3], long_binary_marginals.nodes[parent * 2 + 1],
               kLongTolerance));
    Check(Near(joint[0] + joint[2], long_binary_marginals.nodes[child * 2],
               kLongTolerance));
    Check(Near(joint[1] + joint[3], long_binary_marginals.nodes[child * 2 + 1],
               kLongTolerance));
  }
}
